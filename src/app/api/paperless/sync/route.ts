...
export async function POST(request: NextRequest) {
  try {
    ...
    const correspondentId = process.env.PAPERLESS_CORRESPONDENT_ID
    ...
    if (correspondentId) documentUrl += `&correspondent__id=${correspondentId}`
    if (documentTypeId) documentUrl += `&document_type__id=${documentTypeId}`
